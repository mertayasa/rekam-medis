            ...
            {!! Form::open(['method' => 'POST', 'route' => ['rekam.update_diagnosa', $pasien->id], 'id' => 'editDiagnosaForm']) !!}
                <div class="row">
                    <div class="col-12 mt-2  pb-3 pb-md-0">
                        ...
                        {!! Form::select('diagnosa',  ['Nyeri Akut' => 'Nyeri Akut', 'Nyeri Kronis' => 'Nyeri Kronis'], null, ['class' => 'form-control', 'id' => 'diagnosa', 'x-model' => '$store.diagnosa.data.diagnosa']) !!}
                    </div>
                </div>
                <div class="text-end mt-3">
                    <button type="button" class="btn btn-sm btn-primary" x-on:click="$store.diagnosa.store($event)">
                        Simpan
                    </button>
                    <button type="button" class="btn btn-sm btn-warning" data-edit-rluaran="true" x-on:click="$store.diagnosa.store($event)">
                        Simpan & Edit Luaran
                    </button>
                </div>
            {!! Form::close() !!}
